...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["username"]) && isset($_POST["password"])) {
    if (session_status() === PHP_SESSION_ACTIVE) alert("danger", "You are already logged in.");
        ...
        $stmt = $db--->prepare("SELECT * FROM users WHERE LOWER(username)=? OR LOWER(email)=?;"); $stmt-&gt;bind_param("ss", strtolower($_POST["username"]), strtolower($_POST["username"])); if (!$stmt-&gt;execute()) alert("danger", "An unknown error occurred while logging in."); else { $result = $stmt-&gt;get_result()-&gt;fetch_all(MYSQLI_ASSOC); if (count($result) != 1 || !password_verify($_POST["password"], $result[0]["password"])) alert("danger", "The given email and password combination is incorrect."); else { session_start(); $sessid = session_id(); $stmt = $db-&gt;prepare("INSERT INTO sessions (id, user, expires) VALUES (?, ?, ?);"); $stmt-&gt;bind_param("sis", $sessid, $result[0]["id"], (new DateTime())-&gt;setTimestamp(strtotime("+1 week"))-&gt;format("Y-m-d H:i:s")); if (!$stmt-&gt;execute()) alert("danger", "An unknown error occurred while logging in."); else { session_init(); alert("success", "You have successfully logged in."); echo "
   <script>setTimeout(() => window.location.pathname = '/', 1500);</script>"; } } } } } ?&gt; 
      ...
      <div class="mb-3"> 
       <div class="row justify-content-center"> 
        <div class="col d-flex justify-content-center">
         <button class="btn btn-primary d-block w-50 align-self-center" type="submit">Login</button>
        </div> 
       </div> 
      </div> 
     </form> 
